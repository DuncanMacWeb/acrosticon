...
var findWords = function* (bigramScores, dictionary, stemgrams) {
  ...
  do {
    ...
    if ((stem.words.length > 0) && (stem.letters.length == 0) && (stem.indices.slice(-1) >= (firstletters.length - maxInterval))) {
      let sentance = stem.words.join(" ");
      if (!(sentance in wordsFound)) {
        wordsFound[sentance] = true;
        yield {
          sentance: sentance,
    ...
    for (let lookAhead = minInterval; lookAhead <= maxInterval; lookAhead++) {
      ...
      if (i < firstletters.length) {
        ...
        //newPossibility.score = {'2': (stemgrams[newPossibility.letters] || 0) * newPossibility.letters.length, '3': 0};
        newPossibility.score = {'2': stemgrams[newPossibility.letters] || 0, '3': 0};

        if ((newPossibility.letters in dictionary) && newPossibility.score['2'] <= newPossibility.letters.length) {
...
function printAcrostic(poemWords, indices, sentance) {
  console.log("--------------------\n" + sentance);
  let j = 0;
  let str = "";
  ...
  loadDictionary().then(dictionary => {
    ...
    loadStemgrams().then(stemgrams => {
      ...
      for (let item of generator) {
        printAcrostic(words, item.indices, item.sentance);
        if (n > 10) {
          break;
        }
        n++;
