...
loadBigramCounts().then(bigramScores => {  
  ...
  loadDictionary().then(dictionary => {
    ...
    // possibleStarts tracks the start-of-word permutations (stems) we want to investigate as
    ...
    do {
      ...
      // possibleStart, so we remove it from the possibleStarts array that keeps track of
      // permutations that haven't been exausted yet
      ...
      for (let lookAhead = minInterval; lookAhead <= maxInterval; lookAhead++) {
        ...
        if (i < firstletters.length) {
          ...
          let newPossibility = {
            letters: stem.letters + firstletters[i],
            indices: stem.indices.slice(),
            score: stem.score + 1
          };
