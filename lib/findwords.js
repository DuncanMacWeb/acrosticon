  ...
  do {
    ...
    for (let lookAhead = minInterval; lookAhead <= maxInterval; lookAhead++) {
      ...
      if (i < firstletters.length) {
        ...
        newPossibility.score = {'2': (stemgrams[newPossibility.letters] || 0) * newPossibility.letters.length, '3': 0}; // Prefer longer words

        // TODO: when we found a dictionary word that also is the beginning of a word, split newPossibility into two options
        // one where we use the dictionary word, the other where we aim for the longer word. 
        if ((newPossibility.letters in dictionary) && (stemgrams[newPossibility.letters] || 0) <= 1) {
        ...
        if (newPossibility.score['2'] > 0 && ((newPossibility.letters in dictionary) || (newPossibility.letters.length <= maxWordLength))) {
